...
- **기능명**: 회의록 → GitHub Issue/Project 동기화 + 개발 repo 핸드오프 (`/git-project-sync`, `/dev-handoff`)
- **버전**: v0.3.0
- **작성일**: 2026-06-30
- **최종 검증일**: 2026-06-30
- **담당자**: @feed-mina
...
#### FR-005: 개발 repo 핸드오프 이슈 생성
- **설명**: 승인된 기획 산출물을 대상 개발 저장소의 구현 이슈로 변환한다.
- **입력**: target repo(`owner/name`) + 기능명 또는 `outputs/<날짜>/` 산출물.
- **처리**: `spec.md`, 요구사항, 플로우, 테스트 기준을 요약해 `outputs/<날짜>/dev-handoff.md` 를 생성. 기본은 dry-run이며 승인 후에만 `gh issue create` 를 실행.
- **출력**: 개발 repo 이슈 본문 + 추천 `@claude` 트리거 문구.
- **우선순위**: 필수(Phase 4)
- **예외**:
  - target repo 누락/모호: 중단 후 확인 요청
  - raw 회의록 포함 위험: 중단 후 민감정보 제거 요청

#### FR-006: 개발 repo GitHub Action 봇 템플릿
- **설명**: 개발 저장소가 모바일/원격 `@claude` 댓글, 라벨, 담당자 트리거로 구현 PR을 만들 수 있게 GitHub Action 템플릿을 제공한다.
- **입력**: 개발 repo 의 `.github/workflows/claude-dev-bot.yml`, `CLAUDE.md`, repo secret.
- **처리**: Claude Code Action 이 이슈/PR 컨텍스트를 읽고 branch + PR 을 생성한다.
- **출력**: 구현 branch, draft/ready PR.
- **우선순위**: 필수(Phase 4)
- **가드레일**: merge/deploy/secrets/권한 변경 금지.
...
#### NFR-004: 원격·모바일 운영 안전성
- **요구사항**: 모바일/홈랩/Actions 진입점에서도 동일한 승인 게이트와 PR 기반 변경 흐름을 유지.
- **구현 방식**: `/dev-handoff` dry-run, 개발 repo `CLAUDE.md`, branch protection, PR review.
- **금지**: bot 의 main 직접 push, 직접 deploy, secret/access policy 변경.
...
승인된 기획 산출물(outputs/spec) → /dev-handoff → 개발 repo Issue
        → @claude / label / assignee trigger → GitHub Action → branch + PR
        → human review/merge → 개발 repo CI/CD 또는 GitOps deploy
```
...
- **dev-handoff command**: 승인된 기획 산출물 → 개발 repo 이슈 본문 생성
- **templates/github-actions/claude-dev-bot.yml**: 개발 repo 용 Claude Code Action 템플릿
- **templates/dev-repo/CLAUDE.md**: 개발 repo 봇 운영 가드레일
...
### 흐름 2: 기획 → 개발 repo PR 핸드오프
**선행 조건**: 기획 산출물 승인됨, 대상 개발 repo 에 Claude Action 템플릿 설치됨
**정상 흐름**:
1. 사용자는 `/dev-handoff <target-repo> <기능>` 실행
2. 시스템은 개발 이슈 본문과 추천 `@claude` 트리거를 dry-run 으로 생성
3. 사용자는 본문 검토 후 승인
4. 시스템은 개발 repo 에 `from-planning,claude-ready` 라벨 이슈를 생성
5. 사용자는 모바일/데스크톱에서 `@claude implement...` 댓글 또는 `claude` 라벨로 봇을 트리거
6. Claude Code Action 은 branch + PR 을 생성
7. 사람은 PR 을 리뷰/머지하고, 개발 repo 의 CI/CD 가 배포한다
**예외 흐름**:
- 대상 repo 불명확 → 중단 후 확인
- 개발 repo 에 Action/secret 없음 → 설치 가이드 안내
- 구현 범위 과대/민감 작업 포함 → handoff 범위 축소 요청
...
#### Entity: DevHandoff (`dev-handoff.md`)
```
{
  "source": "spec/output links",
...
기획 승인 → 핸드오프 제안(dry-run) → 승인 → 개발 이슈 → @claude → PR → 사람 머지
```
...
- [ ] TC-003: 승인된 기능 → 개발 repo handoff 이슈 본문 생성
...
- [ ] TC-023: target repo 누락/모호 → handoff 중단 후 확인 요청
- [ ] TC-024: 개발 repo Action/secret 미설정 → 설치 가이드 안내

### 보안 테스트
- [ ] TC-030: 회의록 원문이 이슈 본문/커밋에 없음
- [ ] TC-031: `meetings/raw/` 가 git 추적에서 제외됨
- [ ] TC-032: 제목 내 특수문자/인젝션 문자열 → 안전 처리
- [ ] TC-033: 개발 봇이 main 직접 push/merge/deploy 하지 않음
...
### Phase 4: 리모트·모바일·개발 봇 스캐폴드 ✅
- **산출물**: `/dev-handoff`, `docs/remote-dev-platform.md`, Claude Code Action 템플릿, 개발 repo `CLAUDE.md` 템플릿
- **완료 기준**: 기획 산출물 → 개발 repo 이슈 → `@claude` PR 흐름을 설치 가능한 템플릿으로 제공

### Phase 5: 다운스트림 관통 테스트
- **산출물**: quali-fit 형 개발 repo 에 템플릿 설치 후 문서/소규모 기능 PR 1회 생성
- **완료 기준**: bot 이 PR까지만 만들고 사람 리뷰/머지 게이트가 유지됨
...
| 2026-06-30 | @feed-mina | 리모트·모바일·개발 repo 핸드오프 설계 추가 | v0.3.0 |
